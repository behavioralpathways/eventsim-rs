...
///
/// ```
/// use eventsim_rs::relationship::TrustworthinessFactors;
/// use eventsim_rs::enums::{TrustPath, LifeDomain};
///
...
impl TrustworthinessFactors {
    ...
    /// ```
    /// use eventsim_rs::relationship::TrustworthinessFactors;
    /// use eventsim_rs::enums::LifeDomain;
    ///
...
mod tests {
    ...
    fn add_competence_delta_all_domains() {
        ...
        assert!((factors.competence(LifeDomain::Work).unwrap().delta() - 0.2).abs() < f32::EPSILON);
        assert!(
            (factors.competence(LifeDomain::Health).unwrap().delta() - 0.2).abs() < f32::EPSILON
        );
        assert!((factors.competence_effective() - 0.5).abs() < f32::EPSILON);
    }
    ...
    fn benevolence_decays_faster_than_competence() {
        ...
        assert!(
            factors.benevolence().delta() < factors.competence(LifeDomain::Work).unwrap().delta()
        );
    }
    ...
    fn integrity_decays_slower_than_competence() {
        ...
        assert!(
            factors.competence(LifeDomain::Work).unwrap().delta() < factors.integrity().delta()
        );
    }
    ...
    fn mutable_references_work() {
        let mut factors = TrustworthinessFactors::new();
        factors
            .competence_mut(LifeDomain::Work)
            .unwrap()
            .add_delta(0.1);
    ...
    fn exponential_smoothing_weights_recent_events_higher() {
        ...
        assert!(
            (factors.competence(LifeDomain::Work).unwrap().delta() - expected_ema).abs() < 0.001
        );
    ...
    fn domain_specific_competence_antecedents() {
        ...
        assert!(
            factors
                .competence(LifeDomain::Financial)
                .unwrap()
                .delta()
                .abs()
                < f32::EPSILON
