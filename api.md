...
```rust
use eventsim_rs::state::stability_coefficient;
...
```rust
use eventsim_rs::state::age_plasticity;
...
```rust
use eventsim_rs::state::sensitive_period_modifier;
...
```rust
use eventsim_rs::state::apply_formative_modifiers;
...
```rust
use eventsim_rs::state::effective_base_at;
...
- **Longitudinal Tests**: See `tests/longitudinal/` for complex multi-year simulation examples
- **Rust API Docs**: Run `cargo doc --open` for generated documentation
