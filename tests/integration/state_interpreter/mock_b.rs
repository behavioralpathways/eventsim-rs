...
fn mock_b_person_in_crisis() {
    ...
    entity
        .individual_state_mut()
        .needs_mut()
        .add_stress_delta(0.6);
    entity
        .individual_state_mut()
        .needs_mut()
        .add_fatigue_delta(0.6);
    entity
        .individual_state_mut()
        .needs_mut()
        .add_purpose_delta(-0.7);
    entity
        .individual_state_mut()
        .social_cognition_mut()
        .add_loneliness_delta(0.6);
    entity
        .individual_state_mut()
        .social_cognition_mut()
        .add_perceived_reciprocal_caring_delta(-0.6);
    entity
        .individual_state_mut()
        .mental_health_mut()
        .add_depression_delta(0.7);
    ...
    assert_eq!(
        interpreter.interpretations().get("valence").unwrap(),
        "feeling very negative"
    );
    assert_eq!(
        interpreter.interpretations().get("arousal").unwrap(),
        "very low energy"
    );
    assert_eq!(
        interpreter.interpretations().get("dominance").unwrap(),
        "feeling very out of control"
    );
    assert_eq!(
        interpreter.interpretations().get("stress").unwrap(),
        "experiencing severe stress"
    );
    assert_eq!(
        interpreter.interpretations().get("fatigue").unwrap(),
        "extremely fatigued"
    );
    assert_eq!(
        interpreter.interpretations().get("purpose").unwrap(),
        "lacks sense of purpose"
    );
    assert_eq!(
        interpreter.interpretations().get("loneliness").unwrap(),
        "feeling very lonely"
    );
    assert_eq!(
        interpreter
    ...
    assert_eq!(
        interpreter.interpretations().get("depression").unwrap(),
