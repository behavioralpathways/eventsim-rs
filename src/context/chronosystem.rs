        ...
        let recent_magnitude: f64 = self
            ...
            .filter(|tp| {
                if current_timestamp >= tp.timestamp {
                    (current_timestamp - tp.timestamp).as_days() <= RECENT_TURNING_POINT_WINDOW_DAYS
                } else {
                    false
                }
            })
...
mod tests {
    ...
    fn chronosystem_cohort_effect_weight_tracks_era_alignment() {
        ...
        assert!((chrono.cohort_effect_weight(BirthEra::Stability) - 0.0).abs() < f64::EPSILON);

        chrono.cohort_effects_mut().birth_era = BirthEra::Stability;
        assert!((chrono.cohort_effect_weight(BirthEra::Stability) - 1.0).abs() < f64::EPSILON);
        assert!((chrono.cohort_effect_weight(BirthEra::Crisis) - 0.5).abs() < f64::EPSILON);
    }
