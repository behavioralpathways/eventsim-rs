...
///
/// ```ignore
/// use eventsim_rs::processor::advance_state;
/// use eventsim_rs::state::IndividualState;
...
///
/// ```ignore
/// use eventsim_rs::processor::regress_state;
/// use eventsim_rs::state::IndividualState;
/// use eventsim_rs::types::Duration;
///
...
fn apply_social_cognition_delta(
    ...
    match path {
        ...
        SocialCognitionPath::PerceivedCompetence => {
            if chronic {
                social.perceived_competence_mut().add_chronic_delta(amount);
            } else {
...
///
/// ```ignore
/// use eventsim_rs::processor::apply_event_to_state;
/// use eventsim_rs::state::IndividualState;
...
fn apply_protective_factors(state: &mut IndividualState, event: &Event, severity: f32) {
    use crate::enums::{
        EventPayload, EventTag, EventType, LifeDomain, RealizationType, SupportType,
    };
...
mod tests {
    ...
    fn apply_event_to_state_group_participation_reduces_loneliness() {
        ...
        assert!(
            grouped_state.social_cognition().loneliness().delta()
    ...
    fn reverse_event_from_state_social_exclusion() {
        ...
        state.social_cognition_mut().add_loneliness_delta(0.14); // 0.7 * 0.2
    ...
    fn apply_interpreted_event_to_state_handles_all_paths() {
        ...
        assert!((new_state.social_cognition().loneliness().delta() - 0.25).abs() < 0.001);
        assert!(
            (new_state
                .social_cognition()
                .perceived_reciprocal_caring()
        ...
        assert!((new_state.social_cognition().perceived_liability().delta() - 0.05).abs() < 0.001);
    ...
    fn reverse_interpreted_event_handles_all_paths() {
        ...
        state.social_cognition_mut().add_loneliness_delta(0.25);
        state
            .social_cognition_mut()
        ...
        state.social_cognition_mut().add_self_hate_delta(0.05);
        ...
        assert!(reversed.social_cognition().self_hate().delta().abs() < 0.001);
    ...
    fn apply_social_cognition_delta_perceived_liability_acute() {
        ...
        assert!((state.social_cognition().perceived_liability().delta() - 0.4).abs() < 0.001);
    }
    ...
    fn apply_social_cognition_delta_self_hate_acute() {
        ...
        assert!((state.social_cognition().self_hate().delta() - 0.2).abs() < 0.001);
    }
    ...
    fn apply_social_cognition_delta_perceived_competence_acute() {
        ...
        assert!((state.social_cognition().perceived_competence().delta() - 0.35).abs() < 0.001);
    }
    ...
    fn apply_interpreted_event_needs_purpose_path() {
        ...
        let interpreted =
            make_test_interpreted_event(event, vec![(StatePath::Needs(NeedsPath::Purpose), 0.15)]);
    ...
    fn apply_interpreted_event_moral_violation_flag_set() {
        ...
        let interpreted = make_test_interpreted_event(
            event,
            vec![(
                crate::enums::StatePath::Mood(crate::enums::MoodPath::Valence),
                -0.15,
            )],
        );
