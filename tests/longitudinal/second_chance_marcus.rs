...
fn second_chance_marcus() {
...

    {
        ...
        println!(
            "Acquired Capability: {:.3} (building from violence exposure)",
            ac
        );
    ...
    let ac_after_shooting;
    {
        ...
        let neuroticism = state.get_effective(StatePath::Hexaco(HexacoPath::Neuroticism));
        ac_after_shooting = state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));

        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Neuroticism: {:.3} (severe increase from trauma)",
            neuroticism
        );
        println!(
            "Acquired Capability: {:.3} (high from death exposure)",
            ac_after_shooting
        );
    }
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Conscientiousness: {:.3} (increased from forced responsibility)",
            conscientiousness
        );
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Honesty-Humility: {:.3} (decreased from criminal activity)",
            honesty_humility
        );
        println!(
            "Thwarted Belongingness: {:.3} (reduced - has gang 'family')",
            tb
        );
    }
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Neuroticism: {:.3} (high from cumulative trauma)",
            neuroticism
        );
...

    {
        ...
        let conscientiousness =
            state.get_effective(StatePath::Hexaco(HexacoPath::Conscientiousness));
        let self_worth_val =
            state.get_effective(StatePath::MentalHealth(MentalHealthPath::SelfWorth));

        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!("Openness: {:.3} (increasing from education)", openness);
        println!(
            "Conscientiousness: {:.3} (increasing from discipline)",
            conscientiousness
        );
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Hopelessness: {:.3} (elevated from rejections)",
            hopelessness
        );
    }
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Honesty-Humility: {:.3} (beginning recovery)",
            honesty_humility
        );
    }
...

    {
        ...
        let final_honesty_humility =
            state.get_effective(StatePath::Hexaco(HexacoPath::HonestyHumility));

        let final_ac = state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        ...
        println!("\n--- MENTAL HEALTH STATE ---");
        println!(
            "Acquired Capability: {:.3} (high from trauma history)",
            final_ac
        );
        ...
        println!(
            "  AC after childhood shooting (age 7): {:.3}",
            ac_after_shooting
        );
