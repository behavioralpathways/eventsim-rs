...
fn riverside_threads() {
    ...
    // Verify valence reduction after hours cut
    {
        ...
        assert!(
            valence < 0.1,
            "Hours cut should reduce valence. Got: {}",
            valence
        );
    }
    ...
    // Note: Valence has 6-hour half-life, so check at +6 hours (1 half-life) to see effect
    {
        ...
        assert!(
            valence < 0.0,
            "Layoff should crash valence. Got: {}",
            valence
        );
        assert!(
            dominance < 0.0,
            "Layoff should reduce dominance. Got: {}",
            dominance
        );
    }
    ...
    // Note: Check at +6 hours (1 half-life) to see valence effect before decay erases it
    {
        ...
        let ac = grace_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));

        assert!(
            valence < 0.0,
            "Shooting should crash valence. Got: {}",
            valence
        );
        assert!(
            arousal > 0.01,
    ...
    // Verify TB increase from withdrawal
    {
        ...
        let tb_before = malik_pre_withdrawal.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        let tb_after = malik_post_withdrawal.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
    ...
    for person in [
        &ava_id, &malik_id, &tasha_id, &nora_id, &luis_id, &grace_id, &priya_id,
    ] {
    ...
    // Verify TB reduction after community meal
    {
        ...
        let tb_after = malik_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
    ...
    // Verify TB increase after marital conflict
    {
        ...
        let tb_before = malik_pre_conflict.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        let tb_after = malik_post_conflict.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
    ...
    // Verify grief response (check immediately, as valence decays with 6-hour half-life)
    {
        ...
        assert!(
            valence_immediate < -0.08,
            "Bereavement should crash valence immediately. Got: {}",
            valence_immediate
        );
    }
    ...
    // Verify TB reduction after sustained support
    {
        ...
        let tb_after = luis_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
    ...
    // Malik: Job loss -> withdrawal -> retraining -> employment recovery
    {
        ...
        println!(
            "  Unemployment low (Jun 2022): Valence = {:.2}",
            valence_unemployed
        );
        println!(
            "  After new job (Nov 2023): Valence = {:.2}",
            valence_recovered
        );
    ...
    // Here we verify retirement provides positive valence shortly after.
    {
        ...
        let valence_post_retirement =
            luis_post_retirement.get_effective(StatePath::Mood(MoodPath::Valence));

        println!("LUIS ROMERO (Age 54):");
        println!(
            "  After retirement (Feb 2024): Valence = {:.2}",
            valence_post_retirement
        );
        println!(
            "  Note: Community support buffered grief via TB reduction (see earlier assertions)"
        );
    ...
    // AC (Acquired Capability) persists long-term after violence; does not decay like mood
    {
        ...
        let ac_post_shooting = grace_post_shooting.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        let ac_final = grace_final.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
    ...
    // Risk zone detection: TB + PB + AC all elevated = high-risk state
    {
        ...
        let tb = luis_post_death.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        let pb = luis_post_death.get_effective(StatePath::MentalHealth(
            MentalHealthPath::PerceivedBurdensomeness,
        ));
        let ac = luis_post_death.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
