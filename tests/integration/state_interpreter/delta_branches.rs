...
fn delta_arousal_increase() {
    ...
    let interpreter = StateInterpreter::from_state_with_baseline(&modified, &baseline_state);
    assert!(
        interpreter.delta_summary().unwrap().contains("more")
            && interpreter.delta_summary().unwrap().contains("energized")
    );
}
...
fn delta_arousal_decrease() {
    ...
    let interpreter = StateInterpreter::from_state_with_baseline(&modified, &baseline_state);
    assert!(
        interpreter.delta_summary().unwrap().contains("less")
            && interpreter.delta_summary().unwrap().contains("energized")
    );
}
...
fn delta_purpose_increase() {
    ...
    assert!(interpreter
        .delta_summary()
        .unwrap()
        .contains("stronger sense of purpose"));
}
...
fn delta_purpose_decrease() {
    ...
    assert!(interpreter
        .delta_summary()
        .unwrap()
        .contains("weaker sense of purpose"));
}
...
fn delta_perceived_reciprocal_caring_increase() {
    ...
    modified
        .social_cognition_mut()
        .add_perceived_reciprocal_caring_delta(0.3);

    let interpreter = StateInterpreter::from_state_with_baseline(&modified, &baseline_state);
    assert!(interpreter
        .delta_summary()
        .unwrap()
        .contains("more cared for"));
}
...
fn delta_perceived_reciprocal_caring_decrease() {
    ...
    assert!(
        ...
        "Expected 'less cared for' in delta: {:?}",
        delta
    );
}
