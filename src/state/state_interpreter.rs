...
impl StateInterpreter {
    ...
    pub fn from_state_with_baseline(state: &IndividualState, baseline: &IndividualState) -> Self {
        ...
        interpretations.insert("valence".to_string(), Self::interpret_valence(valence));
        if let Some(delta) = Self::delta_valence(valence, baseline_valence) {
            deltas.push(delta);
        }
        ...
        interpretations.insert("arousal".to_string(), Self::interpret_arousal(arousal));
        if let Some(delta) = Self::delta_arousal(arousal, baseline_arousal) {
            deltas.push(delta);
        }
        ...
        if let Some(delta) = Self::delta_dominance(dominance, baseline_dominance) {
            deltas.push(delta);
        }
        ...
        interpretations.insert("stress".to_string(), Self::interpret_stress(stress));
        if let Some(delta) = Self::delta_stress(stress, baseline_stress) {
            deltas.push(delta);
        }
        ...
        interpretations.insert("fatigue".to_string(), Self::interpret_fatigue(fatigue));
        if let Some(delta) = Self::delta_fatigue(fatigue, baseline_fatigue) {
            deltas.push(delta);
        }
        ...
        interpretations.insert("purpose".to_string(), Self::interpret_purpose(purpose));
        if let Some(delta) = Self::delta_purpose(purpose, baseline_purpose) {
            deltas.push(delta);
        }
        ...
        if let Some(delta) = Self::delta_loneliness(loneliness, baseline_loneliness) {
            deltas.push(delta);
        }
        let prc = state
            .social_cognition()
            .perceived_reciprocal_caring_effective();
        let baseline_prc = baseline
            .social_cognition()
            .perceived_reciprocal_caring_effective();
        interpretations.insert(
            "perceived_reciprocal_caring".to_string(),
        ...
        if let Some(delta) = Self::delta_perceived_reciprocal_caring(prc, baseline_prc) {
            deltas.push(delta);
        }
        let depression = state.mental_health().depression_effective();
        ...
        if let Some(delta) = Self::delta_depression(depression, baseline_depression) {
            deltas.push(delta);
        }
    ...
    pub fn interpretations(&self) -> &HashMap<String, String> {
    ...
    pub fn delta_summary(&self) -> Option<&str> {
        self.delta_summary.as_deref()
    }
    ...
    fn delta_valence(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "happier", "sadder")
    }
    fn delta_arousal(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more energized", "less energized")
    }
    fn delta_dominance(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more in control", "less in control")
    }
    fn delta_stress(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more stressed", "less stressed")
    }
    fn delta_fatigue(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more fatigued", "less fatigued")
    }
    fn delta_purpose(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(
            current,
            baseline,
            "stronger sense of purpose",
            "weaker sense of purpose",
        )
    }
    fn delta_loneliness(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "lonelier", "less lonely")
    }
    fn delta_perceived_reciprocal_caring(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more cared for", "less cared for")
    }
    fn delta_depression(current: f32, baseline: f32) -> Option<String> {
        Self::build_delta_description(current, baseline, "more depressed", "less depressed")
    }
