...
fn resilient_physician_maya() {
    ...
    println!(
        "  Honesty-Humility:  {:.2}",
        baseline_hexaco.honesty_humility()
    );
    ...
    println!("\n=== STAGE 1: Birth (Age 0) ===");
    {
        ...
        let conscientiousness =
            state.get_effective(StatePath::Hexaco(HexacoPath::Conscientiousness));
...

    {
        ...
        let loneliness = state.get_effective(StatePath::SocialCognition(
            eventsim_rs::enums::SocialCognitionPath::Loneliness,
        ));
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Neuroticism: {:.3} (baseline was {:.3})",
            neuroticism, baseline_neuroticism
        );
...

    {
        ...
        let caring = state.get_effective(StatePath::SocialCognition(
            eventsim_rs::enums::SocialCognitionPath::PerceivedReciprocalCaring,
        ));
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Conscientiousness: {:.3} (baseline was {:.3})",
            conscientiousness, baseline_conscientiousness
        );
    ...
    let ac_after_first_death;
    {
        ...
        ac_after_first_death = state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        let neuroticism = state.get_effective(StatePath::Hexaco(HexacoPath::Neuroticism));
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Acquired Capability: {:.3} (increased from death exposure)",
            ac_after_first_death
        );
        println!("Neuroticism: {:.3}", neuroticism);
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Agreeableness: {:.3} (baseline was {:.3})",
            agreeableness, baseline_agreeableness
        );
    ...
    // State immediately after
    {
        ...
        println!(
            "Age: {:.1} years (30 days post-event)",
            state.age_at_timestamp().as_years_f64()
        );
        println!(
            "Neuroticism: {:.3} (immediate post-trauma)",
            neuroticism_immediate
        );
    ...
    // State after settling period (180 days)
    {
        ...
        println!(
            "\nAge: {:.1} years (200 days post-event, after settling)",
            state.age_at_timestamp().as_years_f64()
        );
        println!(
            "Neuroticism: {:.3} (after severe shift settling)",
            neuroticism_settled
        );
...

    {
        ...
        println!(
            "Extraversion: {:.3} (baseline was {:.3})",
            extraversion, baseline_extraversion
        );
...

    {
        ...
        println!("Age: {:.1} years", state.age_at_timestamp().as_years_f64());
        println!(
            "Neuroticism: {:.3} (diminished impact at this age)",
            neuroticism
        );
        println!("Thwarted Belongingness: {:.3}", tb);
    ...
    let final_ac;
    {
        ...
        final_ac = state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        let final_tb = state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        let final_desire =
            state.get_effective(StatePath::MentalHealth(MentalHealthPath::SuicidalDesire));
        let final_risk =
            state.get_effective(StatePath::MentalHealth(MentalHealthPath::AttemptRisk));
        ...
        println!(
            ...
            final_extraversion - f64::from(baseline_extraversion)
        );
        println!(
            "Openness:          {:.3} (baseline: {:.3})",
            final_openness,
        ...
        println!(
            ...
            final_conscientiousness - f64::from(baseline_conscientiousness)
        );
        println!(
            "Neuroticism:       {:.3} (baseline: {:.3}, delta: {:+.3})",
            final_neuroticism,
            baseline_neuroticism,
            final_neuroticism - f64::from(baseline_neuroticism)
        );
        println!(
            "Agreeableness:     {:.3} (baseline: {:.3}, delta: {:+.3})",
            final_agreeableness,
            baseline_agreeableness,
            final_agreeableness - f64::from(baseline_agreeableness)
        );
        println!(
            "Honesty-Humility:  {:.3} (baseline: {:.3})",
            final_honesty,
            baseline_hexaco.honesty_humility()
        );
        ...
        println!("\n--- MENTAL HEALTH STATE ---");
        println!(
            "Acquired Capability: {:.3} (accumulated from trauma exposures)",
            final_ac
        );
        ...
        println!(
            "  Expected: higher than baseline ({:.3})",
            baseline_neuroticism
        );
        ...
        println!(
            "  AC at age 27 (after first patient death): {:.3}",
            ac_after_first_death
        );
        ...
        assert_eq!(
            state.life_stage(),
            eventsim_rs::enums::LifeStage::MatureAdult
        );
...

    {
        ...
        let historical_ac = historical_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        let historical_neuroticism =
            historical_state.get_effective(StatePath::Hexaco(HexacoPath::Neuroticism));

        println!("Historical state at age 25 (before patient death):");
        println!(
            "  AC: {:.3} (should be lower than {:.3} at age 60)",
            historical_ac, final_ac
        );
        println!("  Neuroticism: {:.3}", historical_neuroticism);
        println!(
            "  Regression quality: {:?}",
            historical_state.regression_quality()
        );
