...
use eventsim_rs::enums::{EventType, MentalHealthPath, Species, StatePath};
...
fn high_risk_convergence_all_three_factors_present() {
    ...
    let risk_ac_only;
    {
        ...
        ac = ac_built_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        desire_ac_only =
            ac_built_state.get_effective(StatePath::MentalHealth(MentalHealthPath::SuicidalDesire));
        risk_ac_only =
            ac_built_state.get_effective(StatePath::MentalHealth(MentalHealthPath::AttemptRisk));
    }

    assert!(ac > 0.5, "AC should be elevated after trauma, got {}", ac);
    ...
    let desire_tb_ac;
    {
        ...
        tb = tb_added_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        desire_tb_ac =
            tb_added_state.get_effective(StatePath::MentalHealth(MentalHealthPath::SuicidalDesire));
    }

    assert!(tb > 0.5, "TB should be above threshold, got {}", tb);
    ...
    let desire_no_hopelessness;
    {
        ...
        pb = pb_added_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::PerceivedBurdensomeness,
        ));
        hopelessness_pre = pb_added_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::InterpersonalHopelessness,
        ));
        desire_no_hopelessness =
            pb_added_state.get_effective(StatePath::MentalHealth(MentalHealthPath::SuicidalDesire));
    }

    // PB should be elevated from burden feedback and failures
    assert!(pb > 0.2, "PB should be elevated, got {}", pb);
    ...
    let risk_final;
    {
        ...
        tb_final = all_factors_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::ThwartedBelongingness,
        ));
        pb_final = all_factors_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::PerceivedBurdensomeness,
        ));
        hopelessness_final = all_factors_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::InterpersonalHopelessness,
        ));
        ac_final = all_factors_state.get_effective(StatePath::MentalHealth(
            MentalHealthPath::AcquiredCapability,
        ));
        desire_final = all_factors_state
            .get_effective(StatePath::MentalHealth(MentalHealthPath::SuicidalDesire));
        risk_final =
            all_factors_state.get_effective(StatePath::MentalHealth(MentalHealthPath::AttemptRisk));
    }

    // All factors should be present (may have different magnitudes)
    assert!(tb_final > 0.25, "TB should be elevated, got {}", tb_final);

    // PB is multiplicative (liability * self_hate), so may be lower
    assert!(pb_final >= 0.0, "PB should be computed, got {}", pb_final);
